...
function Window:CreateHomeTab(HomeTabSettings)
    ...
    -- Методы для создания элементов - просто перенаправляем к существующим методам Tab
    function HomeTab:CreateButton(ButtonSettings)
        local fakeTab = { Page = CustomElementsContainer }
        return Window.CreateButton(fakeTab, ButtonSettings)
    end

    function HomeTab:CreateLabel(LabelSettings)
        local fakeTab = { Page = CustomElementsContainer }
        return Window.CreateLabel(fakeTab, LabelSettings)
    end

    function HomeTab:CreateParagraph(ParagraphSettings)
        local fakeTab = { Page = CustomElementsContainer }
        return Window.CreateParagraph(fakeTab, ParagraphSettings)
    end

    function HomeTab:CreateToggle(ToggleSettings, Flag)
        local fakeTab = { Page = CustomElementsContainer }
        return Window.CreateToggle(fakeTab, ToggleSettings, Flag)
    end

    function HomeTab:CreateSlider(SliderSettings, Flag)
        local fakeTab = { Page = CustomElementsContainer }
        return Window.CreateSlider(fakeTab, SliderSettings, Flag)
    end

    function HomeTab:CreateDropdown(DropdownSettings, Flag)
        local fakeTab = { Page = CustomElementsContainer }
        return Window.CreateDropdown(fakeTab, DropdownSettings, Flag)
    end

    function HomeTab:CreateKeybind(BindSettings, Flag)
        local fakeTab = { Page = CustomElementsContainer }
        return Window.CreateKeybind(fakeTab, BindSettings, Flag)
    end

    function HomeTab:CreateInput(InputSettings, Flag)
        local fakeTab = { Page = CustomElementsContainer }
        return Window.CreateInput(fakeTab, InputSettings, Flag)
    end

    function HomeTab:CreateColorPicker(ColorPickerSettings, Flag)
        local fakeTab = { Page = CustomElementsContainer }
        return Window.CreateColorPicker(fakeTab, ColorPickerSettings, Flag)
    end

    function HomeTab:CreateSection(name)
        local fakeTab = { Page = CustomElementsContainer }
        return Window.CreateSection(fakeTab, name)
    end

    function HomeTab:CreateDivider()
        local fakeTab = { Page = CustomElementsContainer }
        return Window.CreateDivider(fakeTab)
    end
